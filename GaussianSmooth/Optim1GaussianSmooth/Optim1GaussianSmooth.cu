...
const unsigned int  GRID_SIZE_X  = 16; // grid x size
const unsigned int  GRID_SIZE_Y  = 16; // grid y size
const unsigned int  IMAGE_WIDTH  = 256; // image width size
const unsigned int  IMAGE_HEIGHT = 256; // image height size
...
void GaussianSmooth_CPU(unsigned char* pInputImg, unsigned char *pOutputImg, unsigned int width, unsigned height)
{
	...
	for (int row = 0; row < height; row++)
	{
		for(int col = 0; col < width; col++)
		{
			...
			for( int i = -KERNEL_SIZE/2; i < KERNEL_SIZE/2; i++)
			{
				for( int j = -KERNEL_SIZE/2; j < KERNEL_SIZE/2; j++)
				{
					...
					else
					{
						...
						temppixel = *(pInputImg + (row+i)*width + col+j);
					}

					// conv op
					sum += temppixel * gs_kernel[i+KERNEL_SIZE/2][j+KERNEL_SIZE/2];
				}
			}
			...
			*(pOutputImg + row * width + col) = final_pixel;
		}
	}
}
...
{
	const int gtid_x = threadIdx.x +blockDim.x * blockIdx.x;
	const int gtid_y = threadIdx.y +blockDim.y * blockIdx.y;
	const int total_size_x = blockDim.x * gridDim.x;
	const int total_size_y = blockDim.y * gridDim.y;

	// use const memory 
	__const__ int gs_kernel[KERNEL_SIZE][KERNEL_SIZE] =
	{
		{1, 4, 7, 4, 1},
		{4, 16, 26, 16, 4},
		{7, 26, 41, 26, 7},
	...
	/*
	 * const int shared_x = blockDim.x + KERNEL_SIZE/2*2;
	 * const int shared_y = blockDim.y + KERNEL_SIZE/2*2;
	 */
	const int shared_x = BLOCK_SIZE_X + KERNEL_SIZE/2*2;
	const int shared_y = BLOCK_SIZE_Y + KERNEL_SIZE/2*2;
	__shared__  int shared[shared_x * shared_y];
	/* extern __shared__  int shared[]; */

	for( int row = gtid_x; row < *height; row += total_size_x)
	{
		for( int col = gtid_y; col < *width; col += total_size_y)
		{
				int sum = 0;

				// ①　read pixel img from global memory to shared memory
				for( int i = -KERNEL_SIZE/2; i < KERNEL_SIZE/2; i++)
				{
					for( int j = -KERNEL_SIZE/2; j < KERNEL_SIZE/2; j++)
					{
						...
						if((row+i) < 0 || (col+j) < 0 ||
								(row+i) > (*height)-1 || (col+j) > (*width)-1)
						{
							shared[(threadIdx.y+i+KERNEL_SIZE/2) * (blockDim.x + KERNEL_SIZE/2*2) + (threadIdx.x+j+KERNEL_SIZE/2)] = 0;
						}
						else
						{
							shared[(threadIdx.y+i+KERNEL_SIZE/2) * (blockDim.x + KERNEL_SIZE/2*2) + (threadIdx.x+j+KERNEL_SIZE/2)] =  *(pInputImg + (row+i)*(*width) + (col+j));
						}
					}
				}
				...
				// ②　do conv in shared memory
				for( int i = 0; i < KERNEL_SIZE; i++)
				{
					for( int j = 0; j < KERNEL_SIZE; j++)
					{

						// conv op in shared memory
						sum += shared[(threadIdx.y+i) * (blockDim.x + KERNEL_SIZE/2*2) + (threadIdx.x+j)]* gs_kernel[i][j];
					}
				}
				...
				// ③　write conv op result to global memory
				int final_pixel = sum/273;
				if(final_pixel < 0)
				{
					*(pOutputImg + row * (*width) + col) = 0;
				}
				else if(final_pixel > 255)
				{
					*(pOutputImg + row * (*width) + col) = 255;
				}
				else
				{
					*(pOutputImg + row * (*width) + col) = final_pixel;
