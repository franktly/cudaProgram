...
const unsigned int  GRID_SIZE_X  = 32; // grid x size
const unsigned int  GRID_SIZE_Y  = 32; // grid y size
...
__global__ void GaussianSmooth_Kernel(unsigned char *pInputImg, unsigned char *pOutputImg, int* width, int* height)
{
	...
	for( int row = tid_x; row < *height; row += total_size_x)
	{
		for( int col = tid_y; col < *width; col += total_size_y)
		{
			int sum = 0;
			int temppixel = 0;
			...
			for( int i = -KERNEL_SIZE/2; i < KERNEL_SIZE/2; i++)
			{
				for( int j = -KERNEL_SIZE/2; j < KERNEL_SIZE/2; j++)
				{
					...
					// conv op
					sum += temppixel * gs_kernel[i+2][j+2];
				}
			}
